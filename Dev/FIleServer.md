...
- on stdio side: unity_code_mcp_bridge_over_file
...
unity_code_mcp_bridge_over_file is a bridge that runs on the stdio side and has the same MCP-facing features as unity_code_mcp_bridge_stdio.py, but instead of HTTP it communicates with the FileServer by writing request files to the watched directory and reading response files.
...
1. MCP client sends request to the stdio bridge (unity_code_mcp_bridge_over_file) using MCP protocol over stdio
...
1. Default request timeout for unity_code_mcp_bridge_over_file is 3 minutes.
