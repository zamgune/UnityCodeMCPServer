...
- `unity_code_mcp_bridge_over_file` on the Python stdio side
...
- `clientId` is generated by `unity_code_mcp_bridge_over_file` at startup and remains stable for the lifetime of that process.
...
### Python side: `unity_code_mcp_bridge_over_file`

`unity_code_mcp_bridge_over_file` owns MCP-over-stdio integration, per-process client identity, request queueing, request file writing, response file waiting, timeout handling, and response forwarding back to the MCP client.
...
sequenceDiagram
    ...
    participant B as unity_code_mcp_bridge_over_file
...
- `unity_code_mcp_bridge_over_file` starts the timeout after successfully creating the request file.
